...
ESSAYS_DATA = {
    "tesla_essay_1": {
        ...
        "badge": "WARP DRIVE ⚡",
        "music": "Cipher2.mp3",
        "bg_screenshots": ["tesla_ref_1", "tesla_ref_2", "tesla_ref_3", "tesla_ref_5", "tesla_ref_7", "tesla_ref_8", "tesla_ref_10"],
        "chapters": {
            1: {"subtitle": "Al Borde del Abismo", "badge": "CRISIS 2008 🚨", "bullets": ["Faltaban 48 horas para la quiebra", "Imposibilidad de pagar nóminas", "El rescate agónico de Elon Musk"]},
            2: {"subtitle": "El Plan Maestro y el Desafío del Model S", "badge": "ESCALA ⚡", "bullets": ["El salto de escala del Model S", "Cadena de suministro hiper-acelerada", "La necesidad de un sistema central"]},
            3: {"subtitle": "El Ingeniero que Dijo No a Elon Musk", "badge": "LIDERAZGO 👤", "bullets": ["La negativa inicial de Jay Vijayan", "El reclutamiento estratégico de Musk", "El desafío de rediseñar la infraestructura"]},
            4: {"subtitle": "La Rigidez de SAP vs la Velocidad de Tesla", "badge": "CONFLICTO ⚙️", "bullets": ["SAP diseñado para procesos lentos", "Tesla cambia componentes a diario", "Costes millonarios por modificación"]},
            5: {"subtitle": "Operación Warp Drive", "badge": "WARP DRIVE 🚀", "bullets": ["Plazo normal: de 2 a 3 años", "El ultimátum de Elon Musk", "Lanzamiento de Warp Drive en 4 meses"]},
            6: {"subtitle": "Integración Total de Gigafábricas", "badge": "AGILIDAD 📊", "bullets": ["Conexión en tiempo real de baterías y ventas", "Eliminación de licencias de millones de dólares", "Ajustes instantáneos en la línea de montaje"]},
            7: {"subtitle": "¿Por Qué el Resto de la Industria No Puede Copiarlo?", "badge": "LECCIÓN 💡", "bullets": ["Dependencia histórica de distribuidores", "Resistencia al cambio en marcas tradicionales", "Por qué SAP sigue dominando la automoción"]}
        }
    }
}
...
def draw_progress_bar(draw_img, x, y, w, h, progress, color=(232, 33, 39, 220), bg_color=(255, 255, 255, 20)):
...
def draw_horizontal_frame(draw, width, height, title, subtitle, bullets, badge, font_title, font_sub, font_badge, blurred_bg_img, progress, vignette_img=None, effect_type="zoom_in"):
    if blurred_bg_img:
        ...
        img_bg = Image.new("RGBA", (width, height), (10, 13, 20, 255))
    ...
    # Glass Panel card layout - Premium Tesla Obsidian style
    panel_w = 1200
    panel_h = 600
    ...
    # Tesla Red & Cyber Cyan accent borders
    draw_img.rounded_rectangle([panel_left, panel_top, panel_right, panel_bottom], radius=24, fill=(10, 13, 20, 180), outline=(232, 33, 39, 80), width=3)
    draw_img.rounded_rectangle([panel_left - 4, panel_top - 4, panel_right + 4, panel_bottom + 4], radius=28, fill=(0, 0, 0, 0), outline=(0, 210, 255, 25), width=2)
    ...
    # Divider line
    draw_img.line([panel_left + 80, panel_top + 220, panel_right - 80, panel_top + 220], fill=(232, 33, 39, 60), width=2)
    ...
    for i, bullet in enumerate(bullets):
        ...
        dot_color = (232, 33, 39, 255) if is_highlighted else (232, 33, 39, 80)
    ...
    # Badge drawing (top right)
    if badge:
        badge_w = int(draw_img.textlength(badge, font=font_badge)) + 40
        badge_h = 44
        badge_x = panel_right - badge_w - 40
        badge_y = panel_top + 40
        draw_img.rounded_rectangle([badge_x, badge_y, badge_x + badge_w, badge_y + badge_h], radius=12, fill=(232, 33, 39, 45), outline=(232, 33, 39, 210), width=2)
    ...
    draw_progress_bar(draw_img, bar_x, bar_y, bar_w, 8, progress, color=(232, 33, 39, 230))
    
    return img_bg

def compile_essays():
    script_path = os.path.join(BASE_DIR, "scripts_tesla_campaign.md")
    
    print("\n====================================================")
    print("DOMINUSBABEL TESLA SAP VIDEO ESSAYS COMPILER")
    print("====================================================\n")
    ...
    # Pre-render static vignette overlay once
    ...
    for key, info in ESSAYS_DATA.items():
        ...
        for idx_ch, (start, end, ch_title) in enumerate(boundaries):
            ...
            if base_img:
                ...
                overlay = Image.new("RGBA", base_img.size, (6, 8, 16, 160))
        ...
        # Concatenate and assemble audio
