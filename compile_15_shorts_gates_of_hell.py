...
VIDEO_SOURCE = os.path.join(PROJECT_DIR, "gameplay_lowres.mp4")
AUDIO_SOURCE = os.path.join(PROJECT_DIR, "gameplay_audio.webm")

# Definición de los 15 Shorts
SHORTS_SPEC = [
    {
        "id": 1,
        "start": "00:01:15",
...
def render_short(spec):
    ...
    print(f"\n[+] Renderizando Short {spec['id']}: {spec['title']} ({spec['start']} -> {spec['to']})")
    ...
    if os.path.exists(AUDIO_SOURCE) and os.path.exists(music_file):
        # 3 inputs: 0 = video, 1 = audio del juego, 2 = música de fondo
    ...
    if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
        ...
        print(f"    [OK] Generado: {output_filename} ({size_mb:.2f} MB)")
        return True
    else:
        print(f"    [ERROR] Fallo al renderizar: {res.stderr[:200] if res.stderr else 'Unknown error'}")
        return False

def main():
    print("=================================================================")
    print("Iniciando compilacion automatica de los 15 Shorts (Format 9:16)")
    print("=================================================================")
    
    success = 0
    for spec in SHORTS_SPEC:
        if render_short(spec):
            success += 1
            
    print(f"\n================================================ metaphysics ==")
    print(f"RESULTADO: {success} de {len(SHORTS_SPEC)} Shorts exportados correctamente.")
    print(f"Carpeta de destino: {PROJECT_DIR}")
    print("================================================================")
