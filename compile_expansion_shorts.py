...
# Populate metadata for all 54 shorts
for cat in ["stoic", "kabbalah", "humanitas"]:
    m_track = "Moorland.mp3" if cat == "stoic" else ("Rites.mp3" if cat == "kabbalah" else "Cipher2.mp3")
    bg_prefix = "stoic_screenshot_" if cat == "stoic" else ("kabbalah_screenshot_" if cat == "kabbalah" else "humanitas_screenshot_")
    cat_badge = "ESTOICISMO 🛡" if cat == "stoic" else ("CÁBALA 🌟" if cat == "kabbalah" else "ÉTICA IA 🌐")
    
    for e_idx in range(1, 4):
        for s_idx in range(1, 7):
            key = f"{cat}_essay_{e_idx}_short_{s_idx}"
            
            # Subtitle/Title logic based on short index
            short_titles = {
                "stoic": [
            ...
            # Map index out of 18 (e_idx and s_idx combined)
            ...
            SHORTS_DATA[key] = {
                ...
                "badge": cat_badge,
                "music": m_track,
                "bg_idx": (s_idx - 1) % 3, # Rotate screenshot index 0, 1, 2
                "bg_prefix": bg_prefix
            }
...
def draw_vertical_short_frame(draw, width, height, title, badge, active_words, font_title, font_sub, font_badge, font_act, font_side, sharp_bg_img, progress, effect_type="zoom_in", frame_idx=0):
    if sharp_bg_img:
    ...
    card_top = 100 + float_offset
    card_right = card_left + card_w
    card_bottom = card_top + card_h
    
    # Glow Border
    draw_img.rounded_rectangle([card_left, card_top, card_right, card_bottom], radius=24, fill=(10, 18, 36, 150), outline=(56, 189, 248, 45), width=2)
...
def compile_expansion_shorts():
    script_path = os.path.join(BASE_DIR, "scripts_expansion_shorts.md")
    
    print("\n====================================================")
    print("EXPANSION SHORTS COMPILER (54 SHORTS)")
    print("====================================================\n")
    ...
    for idx, (key, info) in enumerate(SHORTS_DATA.items()):
        ...
        print(f"Compiling Expansion Short: {key}...")
