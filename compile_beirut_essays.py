...
ESSAYS_DATA = {
    "beirut_essay_1": {
        "essay_num": 1,
        "badge": "DOCUMENTAL 🇱🇧",
        "music": "Moorland.mp3", # Nostalgic/Somber base theme
        "bg_screenshots": ["beirut_ref_1", "beirut_ref_2", "beirut_ref_3", "beirut_ref_4", "beirut_ref_6", "beirut_ref_8", "beirut_ref_10"],
        "chapters": {
            1: {"subtitle": "El Barco de la Discordia", "badge": "EL BUQUE MALDITO 🚢", "bullets": ["La llegada del MV Rhosus en 2013", "Cargamento letal confiscado", "2,750 toneladas de nitrato de amonio"]},
            2: {"subtitle": "El Almacén de la Muerte", "badge": "HANGAR 12 💣", "bullets": ["Descarga insecure del químico", "Almacenamiento sin ventilación", "Negligencia de las autoridades"]},
            3: {"subtitle": "Las Advertencias Ignoradas", "badge": "ADVERTENCIAS 📁", "bullets": ["Alertas de aduanas ignoradas", "Burocracia inerte libanesa", "Alertas de peligro inminente"]},
            4: {"subtitle": "El Incendio en el Hangar 12", "badge": "EL INCENDIO 🔥", "bullets": ["Trabajos de soldadura en el almacén", "Fuegos artificiales almacenados", "El inicio de la catástrofe"]},
            5: {"subtitle": "La Onda Expansiva Supersónica", "badge": "LA DETONACIÓN 💥", "bullets": ["Detonación supersónica masiva", "Nube de condensación Wilson", "Destrucción en kilómetros a la redonda"]},
            6: {"subtitle": "Beirut en Ruinas", "badge": "LA TRAGEDIA 🏥", "bullets": ["Más de doscientas víctimas mortales", "Trescientos mil desplazados", "Hospitales colapsados en escombros"]},
            7: {"subtitle": "La Ira del Líbano", "badge": "LA INDIGNACIÓN 🇱🇧", "bullets": ["Protestas populares masivas", "Dimisión del gobierno libanés", "Impunidad ante la justicia"]}
        }
    }
}
...
def draw_horizontal_frame(draw, width, height, title, subtitle, bullets, badge, font_title, font_sub, font_badge, blurred_bg_img, progress, vignette_img=None, effect_type="zoom_in"):
    ...
    # Crimson orange borders for explosion theme
    draw_img.rounded_rectangle([panel_left, panel_top, panel_right, panel_bottom], radius=24, fill=(12, 16, 26, 170), outline=(239, 68, 68, 60), width=3)
    draw_img.rounded_rectangle([panel_left - 4, panel_top - 4, panel_right + 4, panel_bottom + 4], radius=28, fill=(0, 0, 0, 0), outline=(239, 68, 68, 15), width=2)
    
    # Chapter title and subtitle
    draw_img.text((panel_left + 80, panel_top + 80), title.upper(), font=font_title, fill=(255, 255, 255, 255))
    draw_img.text((panel_left + 80, panel_top + 155), subtitle, font=font_sub, fill=(156, 163, 175, 255))
    
    # Divider line
    draw_img.line([panel_left + 80, panel_top + 220, panel_right - 80, panel_top + 220], fill=(239, 68, 68, 40), width=2)
    ...
    for i, bullet in enumerate(bullets):
        ...
        text_color = (255, 255, 255, 255) if is_highlighted else (156, 163, 175, 120)
        dot_color = (239, 68, 68, 255) if is_highlighted else (239, 68, 68, 80)
    ...
    if badge:
        ...
        draw_img.rounded_rectangle([badge_x, badge_y, badge_x + badge_w, badge_y + badge_h], radius=12, fill=(239, 68, 68, 40), outline=(239, 68, 68, 200), width=2)
    ...
    draw_progress_bar(draw_img, bar_x, bar_y, bar_w, 8, progress, color=(239, 68, 68, 220))
...
def compile_essays():
    ...
    print("\n====================================================")
    print("DOMINUSBABEL BEIRUT VIDEO ESSAYS COMPILER")
    print("====================================================\n")
