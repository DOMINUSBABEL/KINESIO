...
SHORTS_DATA = {
    1: {"bg": "tesla_ref_2", "title": "Tesla a 48 Horas de Quiebra", "music": "Volatile Reaction.mp3"},
    2: {"bg": "tesla_ref_5", "title": "¿Por qué despidieron a SAP?", "music": "Cipher2.mp3"},
    3: {"bg": "tesla_ref_6", "title": "El Ingeniero que Dijo No", "music": "Scheming Weasel.mp3"},
    4: {"bg": "tesla_ref_7", "title": "Warp Drive en 4 Meses", "music": "Cipher2.mp3"},
    5: {"bg": "tesla_ref_8", "title": "Ahorro de Miles de Millones", "music": "Clash Defiant.mp3"},
    6: {"bg": "tesla_ref_10", "title": "¿Por qué BMW no lo Copia?", "music": "Moorland.mp3"}
}
                    ...
                    if "Audio (Voz en off)" in line:
                        for j in range(i+1, min(i+5, len(lines))):
                            ...
                            if next_line.startswith('"') and next_line.endswith('"'):
                                cleaned = re.sub(r'#\w+', '', next_line[1:-1]).strip()
                                shorts_text[idx] = cleaned
                                break
    return shorts_text
...
def draw_vertical_frame(width, height, words, title, font_sub, font_title, font_sub_bold, blurred_bg_img, progress, vignette_img=None, effect_type="zoom_in"):
    ...
    # Brand Top Capsule - Tesla Obsidian style
    brand_w = 900
    brand_h = 100
    brand_left = (width - brand_w) // 2
    brand_top = 100
    draw_img.rounded_rectangle([brand_left, brand_top, brand_left + brand_w, brand_top + brand_h], radius=18, fill=(10, 13, 20, 180), outline=(232, 33, 39, 90), width=2)
    ...
    draw_img.rounded_rectangle([box_left, box_top, box_right, box_bottom], radius=24, fill=(10, 13, 20, 160), outline=(232, 33, 39, 70), width=2)
    ...
    if words:
        ...
        for idx, wd in enumerate(chunk):
            ...
            color = (255, 215, 0, 255) if is_active else (255, 255, 255, 140) # Bright gold for active word
    ...
    draw_img.line([bar_x, bar_y, bar_x + int(bar_w * progress), bar_y], fill=(232, 33, 39, 230), width=8)
...
def compile_shorts():
    ...
    print("\n====================================================")
    print("DOMINUSBABEL TESLA SAP VERTICAL SHORTS COMPILER")
    print("====================================================\n")
