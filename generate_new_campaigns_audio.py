# -*- coding: utf-8 -*-
"""
EDGE-TTS AUDIO & VTT GENERATOR WITH RETRIES FOR CAMPAIGNS 3 & 4
"""
...
async def generate_single_short_audio(sid, voice, text):
    ...
    if os.path.exists(mp3_path) and os.path.exists(vtt_path) and os.path.getsize(mp3_path) > 10000:
        print(f"  [SKIP] Audio already exists for '{sid}'")
        return
...
async def main():
    print("=" * 80)
    print("  EDGE-TTS MASS AUDIO GENERATOR: CAMPAIGNS 3 & 4 (25 SHORTS)")
    print("=" * 80)
    for sid, voice, text in NEW_SHORTS:
        await generate_single_short_audio(sid, voice, text)
    print("\n[ALL SUCCESS] 25 New Audio Tracks and VTT Files Generated!")
