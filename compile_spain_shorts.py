...
SHORTS_DATA = {
    1: {"bg": "spain_trophy", "title": "Ferran Gol de la Gloria", "music": "Clash Defiant.mp3"},
    2: {"bg": "messi_celebration", "title": "Adiós Messi del Mundial", "music": "Moorland.mp3"},
    3: {"bg": "referee_red_card", "title": "Roja Sentencia Argentina", "music": "Severe Tire Damage.mp3"},
    4: {"bg": "spain_stars", "title": "MVP Cerebro Rodri", "music": "Cipher2.mp3"},
    5: {"bg": "spain_team", "title": "Magia Pedri Mediocampo", "music": "Take a Chance.mp3"},
    6: {"bg": "spain_trophy", "title": "España Bicampeona", "music": "Clash Defiant.mp3"},
    7: {"bg": "argentina_stadium", "title": "Dolor Messi Derrota", "music": "Moorland.mp3"},
    8: {"bg": "spain_team", "title": "Pizarra de la Fuente", "music": "Sneaky Snitch.mp3"},
    9: {"bg": "argentina_stadium", "title": "Tensión Prórroga Drama", "music": "Volatile Reaction.mp3"},
    10: {"bg": "referee_red_card", "title": "Resistencia Scaloni", "music": "Clash Defiant.mp3"},
    11: {"bg": "spain_stars", "title": "Ferran Tiburón Clave", "music": "Severe Tire Damage.mp3"},
    12: {"bg": "argentina_stadium", "title": "MetLife Epicentro Fútbol", "music": "Cipher2.mp3"},
    13: {"bg": "spain_team", "title": "Clásico España Argentina", "music": "Take a Chance.mp3"},
    14: {"bg": "referee_red_card", "title": "Colapso Físico Calor", "music": "Volatile Reaction.mp3"},
    15: {"bg": "spain_trophy", "title": "Comparación 2010 vs 2026", "music": "Sneaky Snitch.mp3"},
    16: {"bg": "spain_stars", "title": "Estadísticas Reales Final", "music": "Cipher2.mp3"},
    17: {"bg": "spain_team", "title": "Nueva Generación Oro", "music": "Take a Chance.mp3"},
    18: {"bg": "messi_celebration", "title": "Fin Ciclo Argentina", "music": "Moorland.mp3"}
}
...
def draw_vertical_frame(width, height, words, title, font_sub, font_title, font_sub_bold, blurred_bg_img, progress, vignette_img=None, effect_type="zoom_in"):
    ...
    # Static top branding card (Double neon capsule)
    brand_w = 900
    brand_h = 100
    brand_left = (width - brand_w) // 2
    brand_top = 100
    draw_img.rounded_rectangle([brand_left, brand_top, brand_left + brand_w, brand_top + brand_h], radius=18, fill=(12, 20, 38, 160), outline=(56, 189, 248, 80), width=2)
    draw_img.text((width // 2, brand_top + brand_h // 2), title.upper(), font=font_title, fill=(255, 255, 255, 255), anchor="mm")
    
    # Dynamic middle subtitle box (Glassmorphism container)
    box_w = 900
    box_h = 320
    ...
    # Sinoidal glass elevation shift
    float_offset = int(math.sin(progress * math.pi * 2.0) * 6.0)
    box_top += float_offset
    box_bottom += float_offset
    
    draw_img.rounded_rectangle([box_left, box_top, box_right, box_bottom], radius=24, fill=(12, 20, 38, 140), outline=(56, 189, 248, 50), width=2)
    
    # Sincronización precisa de subtítulos (Active Word Highlighter)
    if words:
        chunk, active_in_chunk = get_current_word_highlights(words, progress)
        
        # Draw subtitle line centered
        spacing = 30
        total_w = 0
        word_widths = []
        ...
        for idx, wd in enumerate(chunk):
            ...
            color = (250, 204, 21, 255) if is_active else (255, 255, 255, 140) # Bright yellow for active, muted white for inactive
    ...
    # Bottom Progress Bar capsule
    bar_w = 900
    bar_x = (width - bar_w) // 2
    bar_y = height - 150
    draw_img.line([bar_x, bar_y, bar_x + bar_w, bar_y], fill=(255, 255, 255, 15), width=8)
    draw_img.line([bar_x, bar_y, bar_x + int(bar_w * progress), bar_y], fill=(56, 189, 248, 220), width=8)
...
def compile_shorts():
    ...
    print("\n====================================================")
    print("DOMINUSBABEL SPAIN VERTICAL SHORTS COMPILER")
    print("====================================================\n")
    ...
    # Pre-render vertical vignette overlay once to save CPU cycles
    ...
    for idx in range(1, 19):
        ...
        if base_img:
            ...
            overlay = Image.new("RGBA", base_img.size, (8, 12, 24, 150))
        ...
        # Mix vocals and background music loop
